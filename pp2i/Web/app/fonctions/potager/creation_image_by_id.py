...
def creation_image_by_id(l_polygones, l_id, size, items, file, id_image,string=""):
    ...
    for polygone in l_polygones:
        ...
        if id_plante == 0:
            draw.polygon(polygone, fill='#dddddd', width=1, outline=None)
        else:
    ...
    img.save(file + str(id_image) + string + '.png', "PNG")
    return None
